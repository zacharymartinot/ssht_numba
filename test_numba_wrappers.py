...
assert all(ind2elm_check) == True, "ind2elm functions do not match"

elm2ind_check = [pyssht.elm2ind(el, m) == sshtn.elm2ind(el, m) for el in range(L) for m in range(-el, el)]

assert all(elm2ind_check) == True, "elm2ind functions do not match"
...
assert np.allclose(py_theta, nb_theta) == True
assert np.allclose(py_phi, nb_phi) == True
...
assert np.allclose(f_py_mw, f_nb_mw) == True
...
assert np.allclose(rec_flm_py_mw, rec_flm_nb_mw) == True
assert np.allclose(rec_flm_nb_mw, flm) == True
...
assert np.allclose(flm_py_re_mw, flm_nb_re_mw) == True
...
assert np.allclose(rec_f_re_py, rec_f_re_nb) == True
...
assert np.allclose(f_py_mwss, f_nb_mwss) == True
...
assert np.allclose(rec_flm_py_mwss, rec_flm_nb_mwss) == True
assert np.allclose(rec_flm_nb_mwss, flm) == True
...
assert np.allclose(flm_py_re_mwss, flm_nb_re_mwss) == True

# MWSS inverse real transform

rec_f_re_py_mwss = pyssht.inverse(flm_py_re_mwss, L, Spin=0, Method='MWSS', Reality=True)
